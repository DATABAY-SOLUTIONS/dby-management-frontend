...
import { Project, TimeEntry, Expense, ProjectAssignment, ExpensePayment } from '../types/project';
...
interface ProjectState {
    ...
    getExpensePayments: (projectId: string, expenseId: string) => Promise<ExpensePayment[]>;
}
...
export const useProjectStore = create<ProjectState>((set, get) => ({
    ...
    addExpensePayment: async (projectId, expenseId, payment) => {
        set({ isLoading: true, error: null });
        try {
            const newPayment = await projectService.addExpensePayment(projectId, expenseId, payment);
            set((state) => ({
                projects: state.projects.map((project) =>
                    project.id === projectId
                        ? {
                            ...
                            expenses: project.expenses.map((expense) =>
                                expense.id === expenseId
                                    ? {
                                        ...
                                        paidAmount: (expense.paidAmount || 0) + payment.amount,
                                        remainingAmount: expense.amount - ((expense.paidAmount || 0) + payment.amount),
                                        status: expense.amount <= ((expense.paidAmount || 0) + payment.amount)
                                            ? 'paid'
                                            : 'partially-paid'
                                    }
                                    : expense
    ...
    updateExpensePayment: async (projectId, expenseId, paymentId, payment) => {
        set({ isLoading: true, error: null });
        try {
            const updatedPayment = await projectService.updateExpensePayment(
                projectId,
                expenseId,
                paymentId,
                payment
    ...
    deleteExpensePayment: async (projectId, expenseId, paymentId) => {
        set({ isLoading: true, error: null });
        try {
            await projectService.deleteExpensePayment(projectId, expenseId, paymentId);
            set((state) => ({
                projects: state.projects.map((project) =>
                    project.id === projectId
                        ? {
    ...
    getExpensePayments: async (projectId, expenseId) => {
        set({ isLoading: true, error: null });
        try {
            const payments = await projectService.getExpensePayments(projectId, expenseId);
            set((state) => ({
                projects: state.projects.map((project) =>
                    project.id === projectId
                        ? {
